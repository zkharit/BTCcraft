...
public class SendPlayerCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if(commandSender instanceof Player){
            ...
            if(sendTo == null){
                player.sendMessage(ChatColor.YELLOW + "Cannot find " + strings[0] + " they must be online");
                return true;
            }
            ...
            if(sendWallet == null){
                player.sendMessage(ChatColor.YELLOW + strings[0] + " Does not have a wallet associated with their account");
                return true;
            }

            //get address from first argument
            LegacyAddress send = LegacyAddress.fromBase58(playerWallet.getParams(), sendWallet.getDepositaddress().toString());
