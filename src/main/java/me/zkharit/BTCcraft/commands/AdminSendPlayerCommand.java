...
public class AdminSendPlayerCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if(sendTo == null){
            commandSender.sendMessage(ChatColor.YELLOW + "Cannot find " + strings[0] + " they must be online");
            return true;
        }

        BTCcraftWallet sendWallet = btccraft.getBTCcrafWalletFromCache(sendTo);

        if(sendWallet == null){
            commandSender.sendMessage(ChatColor.YELLOW + strings[0] + " Does not have a wallet associated with their account");
            return true;
        }

        //get address from first argument
        LegacyAddress send = LegacyAddress.fromBase58(adminWallet.getParams(), sendWallet.getDepositaddress().toString());
